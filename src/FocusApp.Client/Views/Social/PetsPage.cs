...
internal sealed class PetsPage : BasePage
{
    ...
    private readonly ILogger<PetsPage> _logger;

    FlexLayout _petsContainer;
    Image _selectedCheckmark;
    Label _responseMessage;

    public PetsPage(IAPIClient client, IAuthenticationService authenticationService, PopupService popupService, FocusAppContext localContext, IMediator mediator, ILogger<PetsPage> logger)
	{
        _client = client;
    ...
    private async Task<List<PetItem>> FetchPetsFromLocalDb()
    {
        ...
        {
            _logger.LogError(ex, "Error occurred when fetching pets from local DB.");
        }
